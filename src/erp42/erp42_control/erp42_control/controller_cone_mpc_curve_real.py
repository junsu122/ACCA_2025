...
from .mpc_node_unity import MPC
...
class PathHandler():
    def __init__(self, node, path_topic, one_topic):
        ...
        self.db_path = os.path.expanduser("/home/yuseong/db_file/path_data.db")
        self.init_db()
        self.path = False
        self.one = False
        ...
        if msg.data:
            try:
                ...
                with sqlite3.connect(self.db_path) as conn:
                    ...
                    raw_cx, raw_cy = self.cx, self.cy

                    # ✅ 리샘플링 적용
                    self.cx, self.cy = self.resample_path(raw_cx, raw_cy, spacing=0.05)
                    ...
                    n_edge = 2  # 사용할 점 개수
                    x_edge = self.cx[-n_edge:] + self.cx[:n_edge]
                    y_edge = self.cy[-n_edge:] + self.cy[:n_edge]
                    yaw_edge = yaw_list[-n_edge:] + yaw_list[:n_edge]
                    ...
                    # 연결 구간 보간 (예: 10개)
                    t_insert = np.linspace(0, 1, 20, endpoint=False)
                    extra_x = cs_x(t_insert)
                    extra_y = cs_y(t_insert)
                    extra_yaw = cs_yaw(t_insert)
                    extra_speed = [int(speeds[0])] * len(extra_x)
                    ...
                    for i in range(len(extra_x)):
                        cur.execute(
                            "INSERT INTO Path (path_id, x, y, yaw, speed) VALUES (?, ?, ?, ?, ?)",
                            ("A1A2", float(extra_x[i]), float(extra_y[i]), float(extra_yaw[i]), extra_speed[i])
                        )                        
        ...
        if not self.path.one: # first lap
            ...
            if self.decision_last_idx(target_idx):
                ...
                if self.decision_straight(target_idx):
                    ...
                    adapted_speed = self.ss.adaptSpeed(target_speed, hdr, ctr, min_value=1, max_value=2, he_gain=40.0, ce_gain=30.0, he_thr=0.07, ce_thr=0.05)
                    if self.state.v * 3.6 >= adapted_speed:
                        input_brake = (abs(self.state.v * 3.6 - adapted_speed) / 20.0) * 200
                    else:
                        input_brake = 0
...
#######################################################################################################################################################
        ...
        msg = ControlMessage()
        msg.speed = max(0, min(65535, int(round(adapted_speed)) * 10)) #int(adapted_speed)*10 
