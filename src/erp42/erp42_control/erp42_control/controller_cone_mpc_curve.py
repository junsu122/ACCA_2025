            ...
            try:
                with sqlite3.connect(self.db_path) as conn:
                    ...
                    for row in self.way.poses:
                        cx.append(row.pose.position.x)
                        cy.append(row.pose.position.y)

                    raw_kappa = self.compute_curvature(cx, cy)
                    ...
                    for i, row in enumerate(self.way.poses):
                        ...
                        cur.execute(
                            "INSERT INTO Path (path_id, x, y, yaw, speed) VALUES (?, ?, ?, ?, ?) ON CONFLICT(idx) DO UPDATE SET path_id=excluded.path_id, x=excluded.x, y=excluded.y, yaw=excluded.yaw, speed=excluded.speed",
                            ("A1A2", x, y, yaw, speed)
                        )
                ...
                self.node.get_logger().error(f"[done_path] DB error: {e}")
...
def main(args = None):
    ...
    while rclpy.ok():
        try:
            if not mpc_ready and path_tracking.path and first == True:
                first = False
