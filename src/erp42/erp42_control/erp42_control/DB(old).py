...
class DB:
    def __init__(self, db_name):
        ...
        self.db_path = "/home/jinju/db_file" + "/" + db_name
        ...
        self.__cur = (
            self.__conn.cursor()
        )  # 커서( sql 데이터베이스 매니저), sql 명령어들을 실해해준다
    ...
    def read_db_n(
        self, table, *n
    ):  # table(Path or Node) 에 해당하는 데이터를 모두 가져온다, 반환 형태 : [(,,,),(,,,), ....] 데이터 개수는 db에 따라 다름
        n_str = ", ".join(n)
    ...
    def write_db_Path(
        self, data
    ):  # Path에 해당하는 데이터를 Path 테이블에 기록한다 // data = [("",int,float,float,float), (,,,,)  .... ] 과 같은 형태 리스트 인덱스 순으로 기록된다. idx값이 같다면 새로운 데이터로 덮어쓴다
    ...
    def query_from_id_to_path(
        self, id
    ):  # id를 통해 Path 테이블의 x,y,yaw 값을 가져온다, 반환 형태: ros2 nav_msgs/msg/Path
    ...
    def read_db_from_id_to_mission(
        self, id
    ):  # id 를 통해 Node테이블의 mission 정보를 가져온다
    ...
    def deletePath(
        self, path_id
    ):  # id에 해당하는 데이터를 지운다(), foreign key의 제약조건이 CASCADE이므로 노드테이블의 id데이터가 지워지면 같이 지워짐
