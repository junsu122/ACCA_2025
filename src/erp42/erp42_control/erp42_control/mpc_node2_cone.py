...
# ---------------------------------------------------
    ...
    Qk: list = field(
        default_factory=lambda: np.diag([8.0, 8.0, 6.0, 24.0])  # levine sim
        # default_factory=lambda: np.diag([50., 50., 5.5, 13.0])
    )  # state error cost matrix, for the the next (T) prediction time steps [x, y, delta, v, yaw, yaw-rate, beta]
    Qfk: list = field(
        default_factory=lambda: np.diag([12.0,12.0,6.0,30.0])  # levine sim
        # default_factory=lambda: np.diag([50., 50., 5.5, 13.0])
    )  # final state error matrix, penalty  for the final state constraints: [x, y, delta, v, yaw, yaw-rate, beta]
    # ---------------------------------------------------
    ...
    MAX_SPEED: float = 10.0 # maximum speed [m/s] ~ 5.0 for levine sim
...
class MPC(Node):
    ...
    def __init__(self, db):
        ...
        pose_topic = "/pf/viz/inferred_pose" if self.is_real else "/odometry/navsat"
        drive_topic = "/mpc_cmd"
        ...
        self.flag_sub = self.create_subscription(Int32, "flag", self.callback_flag , qos_profile_system_default)
        
        self.path_sub = self.create_subscription(Path, "del_path", self.callback_path, qos_profile_system_default)
    ...
    def callback_path(self, msg):
        if self.flag:
            ...
            for p in msg.poses:
                ...
                cyaw.append(yaw)    
            self.waypoints = np.array(zip(cx,cy,cyaw,15))
            self.flag = False
