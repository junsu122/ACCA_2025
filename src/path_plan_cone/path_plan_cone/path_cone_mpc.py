...
class PathPublisher(Node):
    ...
    def spline_interpolation(self, points, s=1, k=3, spacing=0.1):
        ...
        return interpolated_x, interpolated_y
    # def spline_interpolation(self, points, spacing=0.01, smooth_factor=6.0):
    #     if len(points) < 3:
    #         return points[:, 0], points[:, 1]
    ...
    #     #     interpolated_x = cs_x(uniform_distances)
    #     #     interpolated_y = cs_y(uniform_distances)
    #     # else:
    #     #     us_x = UnivariateSpline(cumulative_distances, x, s=smooth_factor)
    #     #     us_y = UnivariateSpline(cumulative_distances, y, s=smooth_factor)
    #     #     interpolated_x = us_x(uniform_distances)
    #     #     interpolated_y = us_y(uniform_distances)

    #     # 보간기 생성
    #     # if self.flag_3:  # 닫힌 경로라면 periodic 적용
    #     #     cs_x = CubicSpline(cumulative_distances, x, bc_type='periodic')
    #     #     cs_y = CubicSpline(cumulative_distances, y, bc_type='periodic')
    #     # else:
    ...
    #     unique_coords = set()
    #     final_x, final_y = [], []
    #     for xi, yi in zip(interpolated_x, interpolated_y):
    ...
    #     if np.linalg.norm(np.array([final_x[0], final_y[0]]) - np.array([final_x[-1], final_y[-1]])) > 0.5:
    ...
    # def spline_interpolation(self, points, s=5.0, k=3, spacing=0.02):
    #     # if len(points) < 2:
    #     #     return points[:, 0], points[:, 1]
    ...
    #     if self.flag_3:
    #     # 경로가 닫히면 periodic spline 사용
    #         cs_x = CubicSpline(range(len(x)), x, bc_type='periodic')
    #         cs_y = CubicSpline(range(len(y)), y, bc_type='periodic')
    #     else:
    #         cs_x = CubicSpline(range(len(x)), x)
    #         cs_y = CubicSpline(range(len(y)), y)
    ...
    #     # UnivariateSpline으로 보간 (곡률을 고려한 스무딩)
    #     us_x = UnivariateSpline(cumulative_distances, x, s=s, k=k)
    #     us_y = UnivariateSpline(cumulative_distances, y, s=s, k=k)
