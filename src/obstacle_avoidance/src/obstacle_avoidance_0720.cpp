...
private:
    ...
    void timer_callback()
    {
        ...
        //if dstance between point1 and car is 1.5m -> Change obstacle_mission_ to true
        ...
        if (obstacle_mission_)
        {
            ...
            if (!obstacle_pose_valid_) {
                ...
                double distance_to_right_path = std::numeric_limits<double>::max();
                double distance_to_left_path = std::numeric_limits<double>::max();

                if (!poses1_.empty())
                {
                    distance_to_right_path = std::sqrt(std::pow(poses1_.front().position.x - odom_position_->x, 2) + std::pow(poses1_.front().position.y - odom_position_->y, 2));
                }

                if (!poses2_.empty())
                {
                    distance_to_left_path = std::sqrt(std::pow(poses2_.front().position.x - odom_position_->x, 2) + std::pow(poses2_.front().position.y - odom_position_->y, 2));
                }

                
                if (right_path_active_ == false && left_path_active_ == false) 
                {
                    ...
                    else if (distance_to_right_path > distance_to_left_path)
                    {
                        right_path_active_ = false;
                    ...
                    if ((distance_to_right_path < distance_to_left_path) && right_path_active_) // 차와 right_path 와의 거리가 더 가깝고 && right path가 활성화 되어 있을 때
                    {
                        left_path_active_ = false;
                        right_path_active_ = true;
                    }
                    else if (((distance_to_right_path > distance_to_left_path) && left_path_active_)) // 차와 left_path 와의 거리가 더 가깝고 && left path가 활성화 되어 있을 때
                    {
                        right_path_active_ = false;
                        left_path_active_ = true;
                    }
            ...
            else
            {
                ...
                if (distance_to_right_path < distance_to_left_path) // car on the right_path
                {
                    left_path_active_ = true;
                    right_path_active_ = false;
                }
                else // car on the left_path
                {
