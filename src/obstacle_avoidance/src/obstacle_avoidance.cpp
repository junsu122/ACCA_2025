...
private:
    ...
    std::vector<geometry_msgs::msg::Pose> generate_smooth_path(const geometry_msgs::msg::Pose &start, const geometry_msgs::msg::Pose &goal)
    {
        ...
        // Set orientations to point to the next pose
        for (size_t i = 0; i < smooth_path.size() - 1; ++i)
        {
            double dx = smooth_path[i + 1].position.x - smooth_path[i].position.x;
            double dy = smooth_path[i + 1].position.y - smooth_path[i].position.y;
            double yaw = std::atan2(dy, dx);

            tf2::Quaternion q;
            q.setRPY(0, 0, yaw);

            smooth_path[i].orientation = tf2::toMsg(q);
        }

        // Set orientation for the last pose
        if (smooth_path.size() > 1)
        {
            smooth_path.back().orientation = smooth_path[smooth_path.size() - 2].orientation;
        }
        else
        {
            smooth_path.back().orientation = start.orientation;
        }
    ...
    void timer_callback()
    {
        ...
        std::cout << distance_to_point1 << std::endl;
        std::cout << distance_to_point2 << std::endl;

        if (distance_to_point1 <= 3.0)
        // if (distance_to_point1 <= 0.0)
        {
        ...
        if (obstacle_mission_)
        {
            if (obstacle_pose_valid_)
            {
                ...
                if (count_ == 1) //smooth_path는 obstacle이 처음 감지되었을 때 한 번만 계산 / 갱신x
                {
                    ...
                    {
                        int start_index = right_path_info.second + 10;
                    ...
                    {
                        int start_index = left_path_info.second + 10;
